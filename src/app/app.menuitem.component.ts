    ...
    template: `
        <ng-container>
            <div *ngIf="root && item.visible !== false">
                <span class="layout-menuitem-text">{{item.pageName}}</span>
            </div>
            <a [attr.href]="item.url" (click)="itemClick($event)" *ngIf="(!item.routerLink && item.subMenus) && item.visible !== false" (keydown.enter)="itemClick($event)"
               [attr.target]="item.target" [attr.tabindex]="0" [attr.aria-label]="item.label" [ngClass]="item.class" (mouseenter)="onMouseEnter()" pRipple
               [pTooltip]="item.pageName" [tooltipDisabled]="active || !(root && app.isSlim() && !app.isMobile())">
            ...
            <a (click)="itemClick($event)" *ngIf="(item.subMenu && !item.subMenus) && item.visible !== false"
               [routerLink]="item.subMenu.routerLink" routerLinkActive="active-menuitem-routerlink" [routerLinkActiveOptions]="{exact: true}"
               [attr.target]="item.target" [attr.tabindex]="0" [ngClass]="item.class" (mouseenter)="onMouseEnter()" pRipple
               [pTooltip]="item.subMenu.pageName" [tooltipDisabled]="active || !(root && app.isSlim() && !app.isMobile())">
                <i [ngClass]="item.subMenu.icon" class="layout-menuitem-icon"></i>
                <span class="layout-menuitem-text">{{item.subMenu.pageName}}</span>
                <span class="p-badge p-component p-badge-no-gutter" [ngClass]="item.badgeClass" *ngIf="item.badge && !root">{{item.badge}}</span>
                <i class="pi pi-fw pi-angle-down layout-submenu-toggler" *ngIf="item.subMenus"></i>
            </a>
            <ul *ngIf="((item.subMenus && root) || (item.subMenus && active)) && item.visible !== false" [@children]="root ? 'visible' : active ? 'visibleAnimated' : 'hiddenAnimated'">
